...
export class PDFMonthlySummaryService {
  ...
  static generateMonthlySummaryHTML(data: MonthlySummaryData): string {
    ...
    const classTypeRows = data.classTypeBreakdown
      .map(
        (item) => `
            <tr>
    ...
    const paymentMethodRows = data.paymentMethodBreakdown
      .map(
        (item) => `
            <tr>
    ...
    const invoiceChangeColor =
      data.previousMonthComparison.invoiceCountChange > 0
        ? "#16a34a"
        : data.previousMonthComparison.invoiceCountChange < 0
          ? "#dc2626"
          : "#000";
    const revenueChangeColor =
      data.previousMonthComparison.revenueChange > 0
        ? "#16a34a"
        : data.previousMonthComparison.revenueChange < 0
          ? "#dc2626"
          : "#000";
    const percentageChangeColor =
      data.previousMonthComparison.percentageChange > 0
        ? "#16a34a"
        : data.previousMonthComparison.percentageChange < 0
          ? "#dc2626"
          : "#000";
  ...
  static async generatePDF(data: MonthlySummaryData): Promise<string> {
    ...
    try {
      ...
      // Validate PDF buffer
      console.log(
        `📊 PDF Buffer info: length=${pdfBuffer.length} bytes, type=${typeof pdfBuffer}`,
      );
