...
import {
  ...
  getClassTypes,
  getTrainers,
  setAllClassTypesNotShownAsPromo,
  updateClassType,
} from "@/db/actions";
import {
  parseAllowedPaymentMethods,
  createAllowedPaymentMethodsString,
...
export default function LekcePage() {
  ...
  async function handleDeleteClassType(id: number) {
    ...
    setClassTypes((prevClassTypes) =>
      prevClassTypes.filter((ct) => ct.id !== id),
    );
    try {
      setIsLoading(true);
  ...
  async function handleUpdateClassType(e: React.FormEvent<HTMLFormElement>) {
    try {
      ...
      const allowedPaymentMethods =
        createAllowedPaymentMethodsString(paymentMethods);
      ...
      // Handle trainer associations
      const trainerIds = formData
        .getAll("trainers")
        .map((trainerId: any) => parseInt(trainerId, 10))
        .filter((id) => !isNaN(id));
  ...
  async function createNewClassType(e: React.FormEvent<HTMLFormElement>) {
    try {
      ...
      console.log("Selected trainers:", data["trainers"]);

      const fetchedClassType = await createClassType(
        newClassType,
        data["trainers"],
      );
    ...
    <div>
      ...
      <div>
        ...
        >
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium">Text na homepage</label>
            <textarea
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-vertical"
              name="homepageText"
          ...
          <Select
            ...
            labelPlacement="outside"
            name="trainers"
            placeholder="Vyberte trenéry pro tuto lekci"
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <label className="flex items-center space-x-2">
                <input
                  className="rounded stroke-2"
                  defaultChecked={false}
                  disabled={true}
                  name="allowQr"
                  type="checkbox"
                />
                <span className="text-sm line-through">QR platba</span>
              </label>
              <label className="flex items-center space-x-2">
                <input
                  className="rounded"
            ...
            <>
              ...
              <ModalBody>
                ...
                >
                  ...
                  <Input
                    isRequired
                    defaultValue={selectedClassType?.description}
                    label="Krátký text v rezerváku"
                    labelPlacement="outside"
                    maxLength={500}
                    name="description"
                    placeholder="Zadejte krátký popis pro rezervační systém"
                  />
                  ...
                  <Select
                    defaultSelectedKeys={
                      selectedClassType?.trainerClassTypes?.map((tct) =>
                        tct.trainerId.toString(),
                      ) || []
                    }
                    items={trainers}
                    label="Trenéři lekce"
                  ...
                  <div className="space-y-2 col-span-full flex gap-4">
                    <div className="">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {(() => {
                          const paymentConfig =
                            selectedClassType?.allowedPaymentMethods
                              ? parseAllowedPaymentMethods(
                                  selectedClassType.allowedPaymentMethods,
                                )
                              : {
                    ...
                    <div className={"flex-1"}>
                      ...
                      <textarea
                        className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-vertical"
                        defaultValue={
                          selectedClassType?.customEmailMessage || ""
                        }
                        maxLength={1000}
                        name="customEmailMessage"
                        placeholder="Zadejte vlastní zprávu, která se zobrazí v potvrzovacím e-mailu"
                  ...
                  <div className="col-span-full">
                    ...
                    <textarea
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-vertical"
                      defaultValue={selectedClassType?.homepageText || ""}
                      name="homepageText"
                      placeholder="Zadejte text který se zobrazí na homepage (pokud není zadán, použije se popis lekce)"
                      rows={4}
                    />
                  ...
                  <div className="col-span-full">
                    <label className="flex items-center space-x-2">
                      <input
                        className="rounded"
                        defaultChecked={
                          selectedClassType?.isShownOnHomepage || false
                        }
