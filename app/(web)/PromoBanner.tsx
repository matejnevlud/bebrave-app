...
export default function PromoBanner({ classType }: any) {
  ...
  return (
    ...
    >
      <div className="flex-[1] block overflow-visible relative">
        {/* Special event badge */}
        <div className="rotate-12 absolute -top-3 -right-10 z-20 bg-gradient-to-r from-purple-500 to-pink-500 text-white px-3 py-1.5 rounded-full font-semibold text-xs shadow-lg">
      ...
      >
        <h1 className="font-sans font-bold text-3xl sm:text-5xl text-gray-900 dark:text-white text-center md:text-right">
          {classType?.name}
        </h1>
        <div
        ...
        >
          {classType.trainerClassTypes.map((tct: any, idx: number) => (
            <div key={tct.trainer?.id} className="flex items-center gap-3">
              <img
        ...
        <p className="mt-6 text-lg leading-relaxed text-gray-600 dark:text-gray-300 text-center md:text-right">
