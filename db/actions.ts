...
import {
  generateUniqueVoucherCodes,
  normalizeVoucherCode,
  isValidVoucherCodeFormat,
} from "@/lib/utils/voucher-codes";
...
export async function createReservation(
  ...
  try {
    ...
    if (paymentMethod === "credit_card") {
      ...
      const voucherCode = userData?.voucherCode;

      if (!voucherCode) {
        throw new Error("Voucher kód je povinný");
      }

      const validationResult = await validateVoucherCode(
        voucherCode,
        classWithRelations.classTypeId,
      );

      if (!validationResult.valid || !validationResult.voucher) {
        throw new Error(validationResult.error || "Neplatný voucher");
      }
      ...
      await redeemVoucher(validationResult.voucher.id, reservation[0].id);
...
async function sendConfirmationEmail(
  ...
  try {
    ...
    const shouldIncludeInvoice =
      paymentMethod !== "voucher" && paymentMethod !== "kredit";

    if (reservationId && shouldIncludeInvoice) {
...
export async function generateVouchers(
  ...
  try {
    ...
    const existingVouchers = await db.query.vouchersTable.findMany({
      columns: { code: true },
    });

    existingVouchers.forEach((v) => existingCodes.add(v.code));

    const codes = generateUniqueVoucherCodes(count, existingCodes);
    ...
    throw new Error("Nepodařilo se vygenerovat vouchery");
  }
}
...
export async function validateVoucherCode(
  ...
  try {
    ...
    if (!isValidVoucherCodeFormat(normalizedCode)) {
      return { valid: false, error: "Neplatný formát kódu voucheru" };
    }
    ...
    if (!voucher) {
      return { valid: false, error: "Voucher nenalezen" };
    }

    if (voucher.status !== "available") {
      return { valid: false, error: "Voucher již byl použit" };
    }

    const now = new Date();

    if (new Date(voucher.validFrom) > now) {
      return { valid: false, error: "Voucher ještě není platný" };
    }

    if (new Date(voucher.validUntil) < now) {
      return { valid: false, error: "Voucher již vypršel" };
    }

    if (voucher.classTypeId !== null && voucher.classTypeId !== classTypeId) {
      return {
        valid: false,
        error: "Voucher nelze použít pro tento typ lekce",
      };
    }

    const classType = await db.query.classTypesTable.findFirst({
      where: eq(classTypesTable.id, classTypeId),
    });

    if (!classType?.isVoucherEligible) {
      return {
        valid: false,
        error: "Vouchery nelze použít pro tento typ lekce",
      };
    }
    ...
    return { valid: false, error: "Chyba při ověřování voucheru" };
  }
}
...
export async function getVouchers(filters?: {
  status?: string;
  classTypeId?: number;
}): Promise<(Voucher & { classType?: ClassType })[]> {
  try {
    const vouchers = await db.query.vouchersTable.findMany({
      where: filters?.status
        ? eq(vouchersTable.status, filters.status)
        : undefined,
      with: {
        classType: true,
      },
    ...
    return vouchers as (Voucher & { classType?: ClassType })[];
  } catch (error) {
...
export async function getVoucherStats(): Promise<{
  ...
  try {
    const all = await db.query.vouchersTable.findMany();

    const now = new Date();
    const stats = {
      total: all.length,
      available: all.filter(
        (v) => v.status === "available" && new Date(v.validUntil) >= now,
      ).length,
      used: all.filter((v) => v.status === "used").length,
      expired: all.filter(
        (v) => v.status === "available" && new Date(v.validUntil) < now,
      ).length,
    };
    ...
    return { total: 0, available: 0, used: 0, expired: 0 };
  }
}

export async function updateClassTypesVoucherEligibility(
  ineligibleClassTypeIds: number[],
): Promise<void> {
  try {
    await db.update(classTypesTable).set({
      isVoucherEligible: sql`CASE WHEN ${classTypesTable.id} = ANY(${ineligibleClassTypeIds}) THEN false ELSE true END`,
    });
  } catch (error) {
    console.error("Error updating class types voucher eligibility:", error);
    throw new Error("Nepodařilo se aktualizovat nastavení voucherů");
  }
}
