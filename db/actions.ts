...
export async function createReservation(
    ...
    try {
        ...
        if (paymentMethod === "credit_card") {
            ...
            if (!dotyposUserId) throw new Error("Nemůžeme získat ID zákazníka v Dotypos");
            const balance = await dotyposGetCustomerCreditBalanceByUserId(dotyposUserId);

            if (!balance || balance < amount) throw new Error("Nemáte dostatečný kredit pro tuto rezervaci");
...
export async function dotyposGetCustomerCreditBalanceByEmailAndPhone(email: string, phone: string): Promise<any | null> {
    ACCESS_TOKEN = await getNewAccessToken();
    if (!ACCESS_TOKEN) {
        throw new Error("No access token available");
    }
    ...
    try {
        ...
        if (response1.data?.totalItemsCount != 1) return {customer: null, balance: null};
    ...
    try {
        ...
        if (error.response?.status == 404 || error.response?.status == 400) {
            console.log(`Customer credit account not found for email: ${email} and phone: ${phone}`);
        } else {
            console.error(`Error fetching customer account from Dotypos:`, error);
        }
...
async function dotyposGetCustomerCreditBalanceByUserId(customerId: string): Promise<any | null> {
    ACCESS_TOKEN = await getNewAccessToken();
    if (!ACCESS_TOKEN) {
        throw new Error("No access token available");
    }
...
async function dotyposPayWithCredit(customerId: string, amount: number): Promise<boolean> {
    ACCESS_TOKEN = await getNewAccessToken();
    if (!ACCESS_TOKEN) {
        throw new Error("No access token available");
    }
